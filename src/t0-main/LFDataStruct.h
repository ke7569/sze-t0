...
#ifndef BACKTEST_ENGINE_LFDATASTRUCT_H
#define BACKTEST_ENGINE_LFDATASTRUCT_H
...
struct LFMarketDataField
{
    ...
    int         AskVolume5;            //申卖量五
    double      BidPrice6;             //申买价
    int         BidVolume6;            //申买量
    double      AskPrice6;             //申卖价
    int         AskVolume6;            //申卖量
    double      BidPrice7;             //申买价
    int         BidVolume7;            //申买量
    double      AskPrice7;             //申卖价
    int         AskVolume7;            //申卖量
    double      BidPrice8;             //申买价
    int         BidVolume8;            //申买量
    double      AskPrice8;             //申卖价
    int         AskVolume8;            //申卖量
    double      BidPrice9;             //申买价
    int         BidVolume9;            //申买量
    double      AskPrice9;             //申卖价
    int         AskVolume9;            //申卖量
    double      BidPriceA;             //申买价
    int         BidVolumeA;            //申买量
    double      AskPriceA;             //申卖价
    int         AskVolumeA;            //申卖量
    double      aBidPrice[10];
    int         aBidVolume[10];
    double      aAskPrice[10];
...
struct LFL2MarketDataField
{
    ...
    int         BidOrderQty1[50]; //委托明细
    double      BidPrice2;             //申买价二
    double      BidVolume2;            //申买量二
    int         BidCount2;             //实际买总委托笔数二
    //int         BidOrderQty2[50]; //委托明细
    double      BidPrice3;             //申买价三
...
{
    char_31     OrderTime;             //委托时间（秒）
    char_9      ExchangeID;            //交易所代码
    char_31     InstrumentID;          //合约代码
    double      Price;                 //委托价格
    double      Volume;                //委托数量
    char_2      OrderKind;             //报单类型 B S
    int64_t     ApplSeqNum;
    char_2      OrdType; // 订单类别 1=市价 2=限价 U=本方最优
    //COrderIdRemStock Level[50];
    int64_t     OrderNo; //SH独有
    int64_t     BizIndex; //SH独有 业务序列号和成交编号统一
    int64_t     IsLast; // SH数据末尾字段isLast，SZ无该字段时默认置为1
};

struct LFL2TradeField
{
    char_31     TradeTime;             //成交时间（秒）
    char_9      ExchangeID;            //交易所代码
    char_31     InstrumentID;          //合约代码
    double      Price;                 //成交价格
    double      Volume;                //成交数量
...
struct LFQryPositionField
{
    ...
    char_31     InstrumentID;          //合约代码
    char_9      ExchangeID;            //交易所代码
    bool        LocalPosFlag = false; // 添加本地仓位标记，如果被设置为true，表示从本地获取仓位
};
...
struct LFRspPositionField
{
    ...
    int                     TdPosition = 0;// 今开， Position - YdPosition
    int                     TdCancelCount = 0; // 今撤单次数
    int                     TdTraded = 0; // 今买成交量 /今卖成交量
    int                     TdOrderCount = 0;// 买委托次数/卖委托次数

    double                  InitialTotal = 0; // 初始总额
    double                  Available = 0;// 可用资金

    // option
    double StrikePrice;//行权价
    double AvgPrice;//成交均价
    double TotalCommissionFee;//手续费总和
    double ProfitLoss;//平仓盈亏
    int ExpireDate;//过期日
    char CallPut[2];
    ...
    int      RepayQty=0;                 // 归还数量
    int      OrderQty=0;                 // 委托数量
    int      RightsQty=0;                // 未偿还权益数量
    int      OverRightsQty=0;            // 逾期未偿还权益数量
    int      RltOpenQty=0;               // 实时开仓数量
...
struct LFInputOrderField
{
    ...
    char_9                      ExchangeID;            //交易所代码
    char_31                     InstrumentID;          //合约代码
    long                        OrderRef;              //报单引用  xele:OrderLocalID, da:LocalNo
    ...
    int                         IsAutoSuspend;         //自动挂起标志 160
    //int                         iReserved;
    char_30                     MiscInfo;              //委托自定义标签 目前是策略名字
    char_30                     OrderSysID; // xele add, da SystemNo, rem m_MarketOrderToken
    char_13                     ExchangeOrderSysID; // xele add
    char_21                     TradePwd;// da add
    char_30                     OrderNo;// da add --
    LfOrderStatusType           OrderStatus;           //报单状态 da add --
    char                     Reserved[3]; // 128+160=288
};
...
struct LFRtnOrderField
{
    ...
    //char_13               ExchangeOrderSysID; // xele add
    char_30                 OrderNo;// da add --
    char                    ServerFlag;// yisheng add --
    int ChannelIndex; //返回席位索引
};
...
struct LFRtnTradeField
{
    ...
    LfOffsetFlagType    OffsetFlag;            //开平标志
    LfHedgeFlagType     HedgeFlag;             //投机套保标志
    char_30             OrderNo;// da add -- rem:m_ExecutionID
};
...
struct LFOrderActionField
{
    ...
    char_11     ExchangeID;            //交易所代码
    char_16     UserID;                //用户代码
    char_31     OrderSysID;            //报单编号 da:SystemNo
    long        OrderRef;              //报单引用 da:LocalNo
    int         RequestID;             //请求编号
    ...
    char_21     TradePwd; // da add
    char_30     OrderNo; // da add
    char        ServerFlag; // ys add
    LfPosiDirectionType  PosiDirection; // da add
};
...
struct LFQryAccountField
{
    char_11     BrokerID;              //经纪公司代码
    char_19     InvestorID;            //投资者代码
    char_13     AccountID; // only for xele 资金账户
};
...
struct LFRspExchangeStateField
{
    char_11     ExchangeID;    //交易所代码
    char_11		CommodityNo;   //< 品种编号
    char_9      Time;          //15:15:02 ///< 交易所时间
    int	        State;			  ///< 交易所状态
};

///查询报单
struct LFQryOrderField
{
    char_11     BrokerID;              //经纪公司代码
    char_19     InvestorID;            //投资者代码
    char_31     InstrumentID;          //合约代码
...
///查询涨跌
struct LFQryLimitPrice
{
    char_31     InstrumentID;          //合约代码
};
#endif

#endif //BACKTEST_ENGINE_LFDATASTRUCT_H

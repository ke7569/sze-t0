...
public:
    ...
    virtual void on_rsp_order(const LFInputOrderField* data, int request_id, short source, long rcv_time, int errorId=0, const char* errorMsg=nullptr);
    virtual void on_rsp_position(const PosHandlerPtr posMap, int request_id, short source, long rcv_time);
    virtual void on_rsp_exchange_state(const LFRspExchangeStateField *data, short source, long rcv_time);
    virtual void on_rsp_order_action(const LFOrderActionField* data, int request_id, short source, long rcv_time, int errorId = 0, const char* errorMsg = nullptr);

    virtual void on_rsp_limit_price(const LFMarketDataField* data, int request_id, short source, long rcv_time);

    virtual void on_rtn_trade_all(const LFRtnTradeField* data, int request_id, short source, long rcv_time);
    
    virtual void on_rsp_account(const LFRspAccountField* data, int request_id, short source, long rcv_time, int errorId = 0, const char* errorMsg = nullptr);
...
public: // util functions, wrap upon WCStrategyUtil
    ...
    int req_position(short source,  string instrument_id="");
    int cancel_order(short source, int order_id);
    int req_qry_order_info(short source, string instrument_id, string sys_id);
    int req_qry_limit_price(short source, string instrument_id);
    int req_account(short source);

    void set_cc(kungfu::yijinjing::IControlCenter *cc){
        util->set_cc(cc);
    }
public:
    /** default destructor */

...
private:
    ...
    bool local_pos_flag = true;
    std::map<short,std::set<std::string>> codes_filter;
    bool is_filter_flag=false;

public:
    WCStrategyUtil(const string& strategyName);

    void set_pos_flag(bool flag) {
        local_pos_flag = flag;
    }
    ...
    bool filter_md(boost::python::list tickers, short source);
    bool is_ok_code(short source, const char *pcode);
    ...
    int insert_gfa_order(short source, string instrument_id, string exchange_id, int volume, LfDirectionType direction, LfOffsetFlagType offset);
    int req_position(short source, const char *instrument_id = nullptr);
    int cancel_order(short source, int order_id);
    int req_qry_order_info(short source, const string &instrument_id, const string& sys_id);
    int req_qry_limit_price(short source, const string &instrument_id);
    ...
    string get_name(){return strategy_name;}
};

//DECLARE_PTR(WCStrategyUtil);
typedef boost::shared_ptr<WCStrategyUtil> WCStrategyUtilPtr;

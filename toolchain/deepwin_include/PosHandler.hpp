...
#define PH_POSITIONS_TOTAL_KEY    "PosTotal"
#define PH_AVAILABLE_KEY    "Available"
...
#define DIRECTION_NUM       4 /**< Net, Long, Short , Stock(Credit)*/
#define POSITIONS_TOTAL_INDEX_MAX 6
...
public:
    ...
    void init_js(const json& js)
    {
		...
		positions_total = js[PH_POSITIONS_TOTAL_KEY];
		...
		if (js.find(PH_AVAILABLE_KEY) != js.end()) {
			set_available(js[PH_AVAILABLE_KEY].get<double>());
		}
    }
	...
	void set_available(double available)
	{
    ...
    {
        json _js;
		_js[PH_POSITIONS_TOTAL_KEY] = positions_total;
		...
		_js[PH_AVAILABLE_KEY] = available;
        return _js;
    }
	...
	inline json& get_pos_array_total(const string& ticker)
	{
		if (positions_total.find(ticker) == positions_total.end())
		{
			json pos_array_total = json::array();
			for (size_t i = 0; i < DIRECTION_NUM * POSITIONS_TOTAL_INDEX_MAX; i++)
			{
				pos_array_total.push_back((VOLUME_DATA_TYPE)0);
			}
			positions_total[ticker] = pos_array_total;
		}
		return positions_total[ticker];
	}
	...
	inline void add_pos_total(const string& ticker, LfPosiDirectionType dir, VOLUME_DATA_TYPE position, VOLUME_DATA_TYPE yd, VOLUME_DATA_TYPE order, VOLUME_DATA_TYPE cancel, VOLUME_DATA_TYPE traded)
	{
		// add position info
		json& pos_array_total = get_pos_array_total(ticker);
		// Net, Long, Short
		int start_pos = 0;
		if (dir == LF_CHAR_Net) {
		}
		else if (dir == LF_CHAR_Long) {
			start_pos = POSITIONS_TOTAL_INDEX_MAX;
		}
		else {
			start_pos = POSITIONS_TOTAL_INDEX_MAX*2;
		}
		pos_array_total[start_pos] = pos_array_total[start_pos].get<VOLUME_DATA_TYPE>() + position;
		pos_array_total[start_pos+1] = pos_array_total[start_pos + 1].get<VOLUME_DATA_TYPE>() + yd;
		pos_array_total[start_pos+2] = pos_array_total[start_pos].get<VOLUME_DATA_TYPE>() - pos_array_total[start_pos + 1].get<VOLUME_DATA_TYPE>();
		pos_array_total[start_pos+3] = pos_array_total[start_pos + 3].get<VOLUME_DATA_TYPE>() + order;
		pos_array_total[start_pos+4] = pos_array_total[start_pos + 4].get<VOLUME_DATA_TYPE>() + cancel;
		pos_array_total[start_pos+5] = pos_array_total[start_pos + 5].get<VOLUME_DATA_TYPE>() + traded;
	}

	inline void update_pos_total(const string& ticker, LfPosiDirectionType dir, VOLUME_DATA_TYPE position, VOLUME_DATA_TYPE yd, VOLUME_DATA_TYPE order, VOLUME_DATA_TYPE cancel, VOLUME_DATA_TYPE traded, double Available)
	{
		// add position info
		json& pos_array_total = get_pos_array_total(ticker);
		// Net, Long, Short
		...
		pos_array_total[start_pos] = position;
		pos_array_total[start_pos + 1] = yd;
		pos_array_total[start_pos + 2] = pos_array_total[start_pos].get<VOLUME_DATA_TYPE>() - pos_array_total[start_pos + 1].get<VOLUME_DATA_TYPE>();
		pos_array_total[start_pos + 3] = order;
		pos_array_total[start_pos + 4] = cancel;
		pos_array_total[start_pos + 5] = traded;
		available = Available;
	}

    inline void update_stock_credit(const string& ticker, int v[6])
	{
		// add position info
		json& pos_array_total = get_pos_array_total(ticker);
		// Net, Long, Short
		int start_pos = POSITIONS_TOTAL_INDEX_MAX * 3;
		pos_array_total[start_pos] = v[0];
		pos_array_total[start_pos + 1] = v[1];
		pos_array_total[start_pos + 2] = v[2];
		pos_array_total[start_pos + 3] = v[3];
		pos_array_total[start_pos + 4] = v[4];
		pos_array_total[start_pos + 5] = v[5];
	}


	inline std::vector<int> get_pos_all(const string& ticker, double &available)
	{
		available = this->available;
		json& pos_array_total = get_pos_array_total(ticker);
		return pos_array_total.get<std::vector<int> >();
	}
	...
	inline json& get_pos_all_json(const string& ticker)
	{
		return get_pos_array_total(ticker);
	}

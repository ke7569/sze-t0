...
#define MARGIN_RATE_KEY         "margin_rate"
...
inline void from_json(const json& j, FeeDetail& fee)
{
	...
	if (j.find(MARGIN_RATE_KEY) != j.end()) {
		fee.margin_rate = j[MARGIN_RATE_KEY].get<double>();
	}
...
inline void to_json(json& j, const FeeDetail& fee)
{
	...
	j[MARGIN_RATE_KEY] = fee.margin_rate;
    size_t idx = 2;
...
public:
	...
	inline double get_margin_rate(const string& ticker, bool is_stock)
	{
		if (is_stock)
			return 1;
		else
		{
			const FeeDetail* detail = get_future_fee_detail(ticker);
			return detail->margin_rate;
		}
	}

	inline double get_contract_rate(const string& ticker, bool is_stock)
	{
		if (is_stock)
			return 1.0;
		else
		{
			const FeeDetail* detail = get_future_fee_detail(ticker);
			if (detail != nullptr) {
				return detail->margin_rate * detail->contract_multiplier;
			}
			return 1.0;
		}
	}
